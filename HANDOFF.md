...
### GESTIONALE / POS — nuovo progetto separato (Fasi 0–3 fatte) — worktree `../gestionale-admin`, branch `feature/gestionale-admin` (15–21/07/2026)

App **gestionale/POS** nuova e **fisicamente separata** dall'app riparazioni (`src/App.js` NON toccato). Vive nel git worktree `~/gestionale-admin` (branch `feature/gestionale-admin`, **pushato** su `origin`, ultimo commit `2d91423`). Frontend in `gestionale-admin/gestionale-web/` (**Vite + React 19 + TypeScript**), **stesso progetto Supabase** dell'app riparazioni (`rrkvbvkiuwpqevrfcliw`). Gira in locale con `cd ~/gestionale-admin/gestionale-web && npm run dev` (porta 5173). **Non ancora deployato su Netlify.**

- **Sicurezza/dati**: tutte le tabelle nuove nascono con RLS `TO authenticated` (revoca ad anon/public) → un utente autenticato Supabase vede il gestionale, l'app riparazioni (policy `TO public using(true)`) resta invariata. La `service_role` key **mai** nel bundle client (solo Netlify Functions/dev shim). Utente auth per il login: `info@zerrillopreziosi.it` (creato in Supabase Auth). `.env.local` (gitignored, non-`VITE_`) contiene anche `ANTHROPIC_API_KEY` per l'AI.
- **Migrations** in `gestionale-admin/supabase/gestionale/` (001–014, tutte applicate a Supabase): 001 catalogo (products/product_categories/suppliers/metal_prices/inventory_movements + vista product_stock); 002 import (colonne costo/marca/codice_fornitore, suppliers.codice, import_batches/errors); 003 immagini (bucket Storage `product-images` pubblico-lettura + product_images); 004/005 tassonomia categorie ad albero N livelli + attributi dinamici; 006 varianti (product_variants + inventory_movements.variant_id); 007/008 vendite (sales/sale_items/gift_cards/gift_card_transactions + variant_stock + RPC `pos_create_sale`); 009 pagamenti frazionati; 010 resi (RPC `pos_create_return` + sales.tipo/reso_di); 011 permuta (pos_create_sale 4° arg `p_resi`); 012 fuori produzione + prezzo di riserva (compare-at Shopify: % o importo); 013 modifica massiva (RPC `bulk_adjust_price`); 014 `category_corrections` (apprendimento AI few-shot).
- **Fase 1 — Catalogo**: tabelle + tab Prodotti. **Fase 2 — Carico**: form prodotto (prezzo fisso o variabile a peso metallo, attributi per macro-categoria incl. pietre secondarie con colore/purezza solo per Diamante), **import Excel ProWeb** "7A giacenze/costo" (737 prodotti importati) e "listino/LIS 1" (prezzi), tassonomia editabile ad albero, varianti prodotto (per lo stesso SKU fornitore riusato su carature diverse: `codice_interno` univoco), ricerca/filtri, duplica, elimina singolo/multiplo. **Wizard "Carico oreficeria"** a 2 step in loop: prezzo `((oro×0,8)+manifattura/g)×peso×2,1` arrotondato per eccesso al multiplo di 5; SKU gerarchico `OR-BR-001`; barcode Code128; cartellino con codice costo `ZE-8`.
- **Fase 3 — Vendite/POS** (sezione **Cassa**): carrello, cliente opzionale, **pagamenti frazionati** (contanti/carta/bonifico/assegno/satispay), buoni acquisto (emissione+uso), scontrino regalo (stampa doppia), **barcode del numero vendita** sulle ricevute, storico con ristampa, **resi** (tab dedicata con ricerca per barcode/numero/cliente; ripristino giacenza; rimborso contanti/buono) e **permuta** (reso contestuale alla vendita, netto venduti−resi, differenza pagata o rimborsata). Scelta col proprietario: **scontrino fiscale resta al registratore telematico**; il gestionale emette ricevuta di cortesia/regalo/reso.
- **Import fornitori con AI**: in Impostazioni → Fornitori, upload Excel/CSV senza schema fisso → **Claude Haiku** mappa colonna→campo (Netlify Function `ai-map-columns.mts` in prod + middleware dev in `vite.config.ts`; chiave server-side).
- **Catalogazione AI** (Impostazioni → Catalogazione AI): assegna la categoria dai testi via Claude; con "affina" resta nella stessa macro aggiungendo solo la sotto-categoria; **filtri** (ricerca/fornitore/categoria) + **selezione righe** (l'AI processa solo i selezionati); **impara** dalle correzioni (few-shot da `category_corrections`). Server in `server/aiCategorize.ts` + `ai-categorize.mts`.
- **Immagini prodotto da URL** (dettaglio prodotto "+ Da URL" e Impostazioni → Import → "Immagini da URL"): l'immagine viene **scaricata e salvata in modo permanente** su Supabase Storage (non solo referenziata). Proxy CORS server-side: `server/fetchImage.ts` (limite 10MB + guardia SSRF host privati) esposto da `netlify/functions/fetch-image.mts` (prod) e middleware Vite (dev, ritorna binario); il client carica poi su Storage con la sessione autenticata (nessuna service_role). Import massivo: file Excel/CSV con colonna SKU + colonne URL, barra di progresso, dedup nel file, conteggio download falliti (`lib/importImmagini.ts`).
- **Filtri catalogo prodotti**: oltre a ricerca/categoria/stato, filtri per **fornitore** e per **marca** (marche derivate dai prodotti a catalogo).
- **Modifica massiva** (Impostazioni → Modifica massiva): cambio categoria/fornitore/marca/stato e aumenti/riduzioni prezzo (% o importo) su selezione, via RPC `bulk_adjust_price`.
- **Fix categorie**: nell'editor categorie ogni voce a **qualsiasi livello** mostra il campo "+ Aggiungi" (prima una sotto-categoria foglia non permetteva di aggiungere figli).
- **Sezione Impostazioni**: Fornitori (CRUD + import AI), Clienti (sola lettura, tabella condivisa), Categorie articoli, Catalogazione AI, Modifica massiva, Import prodotti (Giacenze 7A / Listino LIS 1 / Fuori produzione / **Immagini da URL**), Integrazioni/API (placeholder).
- **LEZIONE build**: il gate reale è **`npm run build`** (`tsc -b`), NON `tsc --noEmit` che sul tsconfig-solution di Vite non controlla nulla (causò un white-screen da import mancante).
- **Nota**: il gestionale attuale del negozio è **Proyes** (non ProWeb; i report import sono generici).
- **Da fare**: Fase 4 quotazione metalli (GoldAPI — serve API key — + job Netlify, auto-compila la quotazione oro del wizard); Fase 5 stampa reale ZPL + server stampa; Fase 6 Shopify (con varianti). Per il deploy Netlify servono le env: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `ANTHROPIC_API_KEY` (poi `SUPABASE_SERVICE_ROLE_KEY`, `GOLDAPI_KEY`, `SHOPIFY_*`).

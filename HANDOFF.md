...
### GESTIONALE / POS — nuovo progetto separato (Fasi 0–3 fatte) — worktree `../gestionale-admin`, branch `feature/gestionale-admin` (15–17/07/2026)

App **gestionale/POS** nuova e **fisicamente separata** dall'app riparazioni (`src/App.js` NON toccato). Vive nel git worktree `~/gestionale-admin` (branch `feature/gestionale-admin`, **pushato** su `origin`, ~11 commit). Frontend in `gestionale-admin/gestionale-web/` (**Vite + React 19 + TypeScript**), **stesso progetto Supabase** dell'app riparazioni (`rrkvbvkiuwpqevrfcliw`). Gira in locale con `cd ~/gestionale-admin/gestionale-web && npm run dev` (porta 5173). **Non ancora deployato su Netlify.**
...
- **Sezione Impostazioni**: Fornitori (CRUD + import AI), Clienti (sola lettura, tabella condivisa), Categorie, Import prodotti, Integrazioni/API (placeholder).

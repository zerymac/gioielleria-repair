...
- **Wizard**: `form.fotos` = array di dataURL (sostituisce `fotoUrl`/`fotoBlob`); input `multiple`; `PhotoGrid` con ✕ (senza conferma, non ancora salvate); bozza include le foto; al salvataggio upload di tutte sulla prima riparazione del lotto (`repairs/<id>.jpg`, `<id>-1.jpg`, …), toast se qualcuna fallisce.
- **Dettaglio**: `PhotoGrid` (miniature, tap = lightbox con ‹ › e contatore, ✕ = "Eliminare? Sì/No"); "📷 Aggiungi foto" multiplo con nomi `<id>-<timestamp>[-k].jpg`; elimina = scheda aggiornata subito + `remove` sullo storage (toast arancione se il file non viene rimosso).
- **Test**: `app.photo.test.js` → 4 test (bozza con 2 foto + rimozione nel wizard + upload; annulla; dettaglio aggiungi 2/elimina 1 con `remove`; retro-compat solo `foto_url`). `fakeSupabase.storage` ora traccia `upload`/`remove` in `__fake.storageOps`. Suite: **42 passati, 1 skip**. Build ok.

### Sessione 17/09/2026 — foto persa sulla R2026-0578: bozza wizard + foto dal dettaglio (commit locale, DA PROVARE SU iPHONE prima del push)
- **Diagnosi**: la policy funziona (570/573/576/577/579 con foto). Per la 578 NESSUN upload è mai partito: dai log dell'iPhone (iOS 18.7, Massi) alle 09:45 apre il wizard e crea il cliente, alle 09:46:31 l'app si **ricarica da zero** (Safari al ritorno dalla fotocamera), alle 09:48 salva la 578 ricompilata senza foto. Il wizard non aveva bozza e non c'era modo di aggiungere la foto dopo.
- **`compressImage`** ora risolve SEMPRE (onerror/toBlob null → `null`); `preparePhoto(file)` ripiega sul file originale se è un'immagine, altrimenti toast "Foto non leggibile". Prima una foto non decodificabile lasciava la Promise appesa e la riparazione veniva salvata senza foto in silenzio. Preview e `form.fotoUrl` ora sono dataURL (prima objectURL: `catalogAI` faceva `split(",")[1]` su un `blob:` URL → base64 inesistente).
- **Bozza wizard** (`localStorage.repairWizardDraft`, TTL 12h): salvata a ogni modifica di form/step (foto come dataURL, `fotoBlob` ricostruito con `dataURLToBlob` al ripristino), ripristinata al mount con toast "Bozza riparazione ripristinata", cancellata al "Crea riparazione" e all'"✕ Annulla".
- **Dettaglio riparazione**: pulsante "📷 Aggiungi foto dell'oggetto" / "Sostituisci foto" (solo se `onFieldChange` è passato): `preparePhoto` → `uploadPhoto(blob, r.id)` → `onFieldChange(id,{fotoUrl: url+"?v="+Date.now()})` (stesso path su storage, il `?v=` evita la cache). Toast rosso se l'upload fallisce.
- **Test**: nuova suite `src/__audit__/app.photo.test.js` (3 test: bozza dopo remount + upload al salvataggio, annulla butta la bozza, foto dal dettaglio). jsdom non decodifica immagini: `window.Image` stubbato con `onerror` → esercita il fallback al file originale. Suite: **41 passati, 1 skip**. Build ok.
...
- **Avviso operatore**: `uploadPhoto` ora memorizza l'errore (`_lastPhotoError`) e `handleSaveRepair` mostra un toast rosso 7s "⚠️ Foto non salvata (…). Riparazione registrata senza foto." (`showToast` ha ora il terzo parametro `ms`). La riparazione si salva comunque. Build ok. Pushato su `main` il 16/09 (deploy Netlify).
- **Test riallineati (16/09, commit test-only)**: i 10 fallimenti erano tutti test vecchi, nessun bug: mock WA `initialize()` ora restituisce una Promise (dal fix fa8a059 il bot fa `.catch`), B4 aggiornato (il riparatore non riceve più il prezzo, 6563e44), E4 usa regex sul testo esteso, `public-pages.test.js` riscritto contro le RPC `get_repair_status`/`get_quote`/`respond_quote` (M10 e C1/M8 ora verificano che i bug siano CHIUSI). Suite: **38 passati, 1 skip, 0 falliti**. Le foto delle riparazioni create prima del 15/09 sono perse (mai arrivate al server).
...
- **RILASCIO ESEGUITO (05/09/2026 pomeriggio, con conferma del proprietario a ogni passo)**: sito Netlify `zerrillo-riparazioni` (team zerymac, id `7c9610b2-…`) collegato dal proprietario al repo, branch `feature/online`; env `REACT_APP_SUPABASE_URL/KEY` impostate, `ANTHROPIC_API_KEY` a carico del proprietario (non verificata). Test da fuori LAN ok: login, etichetta via `print_jobs` (done in 4s), WhatsApp via `wa_jobs` (sent in 13s, al numero del proprietario per R2026-0528). `feature/online` mergiato ff in `main` (594592d) e pushato; cartella live aggiornata; LaunchAgent `com.zerrillo.reactapp` scaricato e plist spostato in `backup/launchagents-disattivati/`; print server riavviato (WA connesso, code attive). **Lockdown SQL APPLICATO** (migration `fase2_lockdown_online`, 05/09/2026 ~18:40, dopo che il proprietario è passato alla modalità con conferma: in auto mode il classificatore lo bloccava). Verificato: anon su `repairs`/`customers`/`orders` via REST → **401**; RPC `get_repair_status`/`get_quote` con token finto → 200 `[]` (pagine pubbliche ok); policy live = solo `authenticated_all` sulle 6 tabelle + `repair_photos_insert/update` (authenticated) su storage; app Netlify 200. **C1 CHIUSO.** Rollback: `fase2-lockdown-online-rollback.sql`. Netlify: branch di produzione spostato su `main` (via Chrome, deploy da `main` ok). Chiave Anthropic: il proprietario l'ha **ruotata** e messa su Netlify come `ANTHROPIC_API_KEY`; AI provata in-app e funzionante. La vecchia `REACT_APP_ANTHROPIC_KEY` nel `.env` del Mac mini è revocata e inutilizzata (il print server non la legge): si può togliere quando capita. **MIGRAZIONE COMPLETA.**
